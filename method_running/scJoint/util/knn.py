...
def KNN(config, neighbors = 30, knn_rna_samples = 20000, output_folder='output'):
    # read rna embeddings and predictions
    print('[KNN] Read RNA data')
    db_name = os.path.basename(config.rna_paths[0]).split('.')[0]
    rna_embeddings = np.loadtxt(output_folder + db_name + '_embeddings.txt')
    rna_predictions = np.loadtxt(output_folder + db_name + '_predictions.txt')
    ...
    for i in range(1, len(config.rna_paths)):
        ...
        rna_embeddings = np.concatenate((rna_embeddings, np.loadtxt(output_folder + db_name + '_embeddings.txt')), 0)
        rna_predictions = np.concatenate((rna_predictions, np.loadtxt(output_folder + db_name + '_predictions.txt')), 0)
    ...
    atac_embeddings = np.loadtxt(output_folder + db_name + '_embeddings.txt')
    atac_predictions = np.loadtxt(output_folder + db_name + '_predictions.txt')
    ...
    for i in range(1, len(config.atac_paths)):
        ...
        em = np.loadtxt(output_folder + db_name + '_embeddings.txt')
        pred = np.loadtxt(output_folder + db_name + '_predictions.txt')
    ...
    atac_predict_prob = neigh.predict_proba(atac_embeddings)
    ...
    for i, db_name in enumerate(db_names):
        np.savetxt(output_folder + db_name + '_knn_predictions.txt', atac_predict[cnt:cnt+db_sizes[i]])
        np.savetxt(output_folder + db_name + '_knn_probs.txt', conf_scores[cnt:cnt+db_sizes[i]])
        print('[KNN] save prob matrix')
        np.savetxt(output_folder + db_name + '_knn_probs_all.txt', atac_predict_prob[cnt:cnt+db_sizes[i]])
        cnt += db_sizes[i]
