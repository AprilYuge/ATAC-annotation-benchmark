library(peakRAM)
binarize <- FALSE

args = commandArgs(trailingOnly=TRUE)
if (!(length(args) %in% c(3,5,6))) {
      stop("One argument must be supplied (input file).n", call.=FALSE)
}
source("/gpfs/gibbs/pi/zhao/yw599/Multiome/utils.r")
scGCN_path <- "/gpfs/gibbs/pi/zhao/yw599/Multiome/ATAC-annotation-benchmark/method_running/scGCN"
rna.path <- args[1]
atac.path <- args[2]
tmp_path <- args[3]
result.folder <- args[3]
if (length(args) >= 4){
    subset.rna <- args[4]
    if (args[4] == '!') { subset.rna <- NULL }
    subset.atac <- args[5]
    if (args[5] == '!') { subset.atac <- NULL }
} else {
    subset.rna <- NULL
    subset.atac <- NULL
}
if (length(args) == 6) {
    annot.new.rna <- args[6]
} else {
    annot.new.rna <- NULL
}

out <- peakRAM(
    function(){
        #data.path <- "/gpfs/gibbs/pi/zhao/xs272/Multiomics/sc_benchmark/data/BMMC_processed_s3d7"
        # obj.rna <- load.rna(data.path)
        # atac.ga <- load.atac.gene.activity.count(data.path)
        # rna_mat <- obj.rna[["RNA"]][, ]
        # atac_mat <- atac.ga
        rna.data <- load.data.folder(rna.path, subset.rna, annot.new.rna, allow.duplicate = TRUE, binarize=binarize)
        rna_mat <- rna.data$counts
        atac.data <- load.data.folder(atac.path, subset.atac, allow.duplicate = TRUE, binarize=binarize)
        atac_mat <- atac.data$counts

        rna_cell_filter = colSums(rna_mat > 0) >= 200
        rna_mat = rna_mat[, rna_cell_filter]
        rna_gene_filter = rowSums(rna_mat > 0) >= 3
        rna_mat = rna_mat[rna_gene_filter,]

        atac_cell_filter = colSums(atac_mat > 0) >= 200
        atac_mat = atac_mat[, atac_cell_filter]
        atac_gene_filter = rowSums(atac_mat > 0) >= 3
        atac_mat = atac_mat[atac_gene_filter,]

        common.genes <- intersect(rownames(rna_mat), rownames(atac_mat))
        rna_mat <- rna_mat[common.genes, ]
        atac_mat <- atac_mat[common.genes, ]
        rna_mat <- as(rna_mat, "matrix")
        atac_mat <- as(atac_mat, "matrix")
        rna.label <- data.frame(rna.data$annotations)
        rna.label <- rna.label[colnames(rna_mat), , drop = FALSE]
        atac.label <- data.frame(atac.data$annotations)
        atac.label <- atac.label[colnames(atac_mat), , drop = FALSE]
        atac.label[,1] <- rna.label[1,1] # this is used to solve the issue that scGCN can not deal with ATAC-specific cell types
        count.list <- list(rna_mat, atac_mat)
        label.list <- list(rna.label, atac.label)
        dir.create(file.path(tmp_path, 'input'), recursive = TRUE)
        source(file.path(scGCN_path, 'data_preprocess_utility.R'))
        # stopifnot(dir.exists(file.path(tmp_path, 'input', 'Data1.csv')))
        # stopifnot(dir.exists(file.path(tmp_path, 'input', 'Data2.csv')))
        # stopifnot(dir.exists(file.path(tmp_path, 'input', 'Label1.csv')))
        # stopifnot(dir.exists(file.path(tmp_path, 'input', 'Label2.csv')))

        save_processed_data(count.list,label.list,folder=tmp_path)
        # writeLines(dim(rna_mat), tmp_path+'/rna.dim')
    }
)

mem <- out$Peak_RAM_Used_MiB
time <- out$Elapsed_Time_sec
print(paste0('Running time: ', time, ' sec'))
print(paste0('Peak memory usage: ', mem, ' MB'))
write(time, file.path(result.folder, "time.txt"))
write(mem, file.path(result.folder, "memory.txt"))
