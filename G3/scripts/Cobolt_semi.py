...
import time
from cobolt.utils import SingleData_semi, MultiomicDataset_semi
from cobolt.model import Cobolt_semi
import os
...
def run_cobolt(rna_path, atac_path, omics_rna_path, omics_atac_path, result_folder, rna_cell_subset, atac_cell_subset, rna_new_annot):
    ...
    rna_label = read_txt_np(os.path.join(rna_path, 'annotations.txt'))
    rna_label = pd.DataFrame({'celltype': rna_label}, index=rna_barcode)
    label_idx_mapping = {}
    unique_labels = np.unique(rna_label['celltype'])
    for i, name in enumerate(unique_labels):
        label_idx_mapping[name] = i
    print(label_idx_mapping)
    rna_label_int = rna_label['celltype'].replace(label_idx_mapping)    
    rna = SingleData_semi("GeneExpr", "RNA", feature, count, rna_barcode, np.array(rna_label_int))
    ...
    atac_label = read_txt_np(os.path.join(atac_path, 'annotations.txt'))
    atac_label = pd.DataFrame({'celltype': atac_label}, index=atac_barcode)
    atac_label_int = atac_label['celltype'].replace(label_idx_mapping)
    atac_label_int[~atac_label_int.isin(list(label_idx_mapping.values()))] = -1
    atac = SingleData_semi("ChromAccess", "ATAC", feature, count, atac_barcode, np.array(atac_label_int))
    ...
    multirna = SingleData_semi("GeneExpr", "multiomics", feature, count, barcode, np.array([-1]*count.shape[0]))
    ...
    multiatac = SingleData_semi("ChromAccess", "multiomics", feature, count, barcode, np.array([-1]*count.shape[0]))
    multiatac.filter_features(min_count=0, min_cell=0, upper_quantile=0.99, lower_quantile=0.7)

    # Merge the above datasets
    multi_dt = MultiomicDataset_semi.from_singledata(rna, atac, multirna, multiatac)

    # Training
    num_of_class = len(np.unique(rna_label_int))
    if 'Embryo' in result_folder:
        model = Cobolt_semi(dataset=multi_dt, lr=0.0002, n_latent=10, num_of_class=num_of_class)
    elif 'results_binarize/MouseBrain_snap' in result_folder:
        model = Cobolt_semi(dataset=multi_dt, lr=0.0002, n_latent=10, num_of_class=num_of_class)
    elif 'results_balanced_ct/kidney' in result_folder:
        model = Cobolt_semi(dataset=multi_dt, lr=0.0002, n_latent=10, num_of_class=num_of_class)
    elif 'MouseAtlas-brain/Cobolt_semi' in result_folder:
        model = Cobolt_semi(dataset=multi_dt, lr=0.0002, n_latent=10, num_of_class=num_of_class)
    else:
        model = Cobolt_semi(dataset=multi_dt, lr=0.001, n_latent=10, num_of_class=num_of_class)
    model.train()
