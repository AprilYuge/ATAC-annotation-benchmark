...
ref_genome = str(sys.argv[5])
result_folder = str(sys.argv[6])
...
def main():
    ...
    if len(sys.argv) >= 9:
        rna_cell_subset = str(sys.argv[7])
        if rna_cell_subset == '!':
            rna_cell_subset = None
        atac_cell_subset = str(sys.argv[8])
        if atac_cell_subset == '!':
    ...
    rna_counts = csr_matrix(mmread(path.join(omics_rna_path, 'counts.mtx'))).astype(int)
    ...
    atac_counts = csr_matrix(mmread(path.join(omics_atac_path, 'counts.mtx'))).astype(int)
    ...
    # KNN classifier
    rna = rna[rna.obs['dataset'] != 'Multiomics'].copy()
    atac = atac[atac.obs['dataset'] != 'Multiomics'].copy()
