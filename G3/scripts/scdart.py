...
def main():  
    ...
    # Run scDART
    scDART_op = scDART(n_epochs = 500, use_anchor = False, k = 10, batch_size=128,
                       device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    scDART_op = scDART_op.fit(rna_count = rna.X.todense(), atac_count = atac.X.todense(), reg = coarse_reg.values, rna_anchor = None, atac_anchor = None)
