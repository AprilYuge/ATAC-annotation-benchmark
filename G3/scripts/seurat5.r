...
run.seurat5 <- function(rna.path, atac.path, activity.path, result.folder, subset.rna, subset.atac, annot.new.rna) {
    ...
    obj.atac <- load.atac.folder.seuratv5(atac.path, subset.atac, assay='RNA', min.cells=3, min.features=200, binarize=binarize)

    # Perform standard analysis of each modality independently RNA analysis
    if (!grepl('Spleen', rna.path)){
        obj.rna <- NormalizeData(obj.rna)
    } else {
        obj.rna <- SetAssayData(
            object = obj.rna,
            layer = "data",
            new.data = GetAssayData(object = obj.rna, layer = "counts"),
            assay = "RNA" # Specify the assay if needed
        )
    }
 ...
 #   Annotation(obj.atac) <- annotations
    ...
    if (!grepl('Spleen', rna.path)){
        ...
        obj.atac <- SetAssayData(
            object = obj.atac,
            layer = "data",
            new.data = GetAssayData(object = obj.atac, layer = "counts"),
            assay = "ACTIVITY" # Specify the assay if needed
        )
    }
...
out <- peakRAM(
    run.seurat5(rna.path, atac.path, activity.path, result.folder, subset.rna, subset.atac, annot.new.rna)
)
