...
import time
from scalex import SCALEX_semi
import scanpy as sc
import pandas as pd
import numpy as np
...
def main():    
    ...
    adata = SCALEX_semi(data_list=[rna, atac], outdir=result_folder, ignore_umap=True, min_features=0, min_cells=0,
                        normalize_samples=False, log_normalize=False,  batch_categories=['RNA', 'ATAC'], label_name='label')

    X_train = adata[adata.obs.batch=='RNA'].obsm['latent']
    y_train = adata[adata.obs.batch=='RNA'].obs['celltype']
    X_test = adata[adata.obs.batch=='ATAC'].obsm['latent']
