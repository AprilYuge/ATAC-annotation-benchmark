...
run.conos <- function(rna.path, activity.path, result.folder, subset.rna, subset.atac, annot.new.rna) {
    ...
    rna.data <- load.rna.folder(rna.path, subset.rna, annot.new.rna, assay='RNA', min.cells=3, min.features=200, binarize=binarize)
    rna.counts <- rna.data[['RNA']]@counts
    gene.activities.data <- load.atac.folder(activity.path, subset.atac, assay='RNA', min.cells=3, min.features=200, binarize=binarize)
    gene.activities <- gene.activities.data[['RNA']]@counts
    ...
    rna.annotation <- as.character(rna.data$seurat_annotations) 
    names(rna.annotation) <- colnames(rna.counts)
