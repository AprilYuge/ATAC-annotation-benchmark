...
import time
import sys
sys.path.append('/gpfs/gibbs/pi/zhao/yw599/Multiome/cross-domain-simulation')
from network import *
...
def main():
    ...
    # Run model
    FeatureExtractorA, FeatureExtractorB= crossmodalAE_semi(epoch=100, batch_size=128,
                                                            source_trainset=rna.X,
                                                            source_label_trainset=rna_label_int,
                                                            target_trainset=atac.X)
    FeatureExtractorA.to(torch.device("cpu"))
    FeatureExtractorB.to(torch.device("cpu"))
    X_train = FeatureExtractorA(torch.tensor(rna.X))
    X_train = F.normalize(X_train, dim=1,p=2).detach().numpy()
    X_test = FeatureExtractorB(torch.tensor(atac.X))
    X_test = F.normalize(X_test, dim=1,p=2).detach().numpy()
