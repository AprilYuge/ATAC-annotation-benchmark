...
print(len(sys.argv))
assert len(sys.argv) == 5 or len(sys.argv) == 7 or len(sys.argv) == 8, "parameters needed: rna path, atac path, result folder"
...
def main():
    ...
    if len(sys.argv) >= 8:
        ...
        rna_new_annot = None
    ga_cell_subset = atac_cell_subset
    ...
    ga_counts = csr_matrix(mmread(path.join(ga_path, 'counts.mtx')))
    ...
    if binarize:
        ga_counts = (ga_counts > 0).astype('int')

    ga = sc.AnnData(ga_counts)
    # ga.var_names = [i.upper() for i in ga_gene_names]
    ...
    if ga_cell_subset is not None:
        ga_cell_subset = read_txt_np(ga_cell_subset)
        ga = ga[ga_cell_subset]
        ga.obs_names = rename_duplicates(ga.obs_names)
    ...
    rna.obs['batch'] = 'RNA'
    ga.obs['batch'] = 'ATAC'
    adata = MultiMAP.Integration([rna, ga], ['X_pca', 'X_lsi'])

    # KNN classifier
    X_train = adata[adata.obs.batch=='RNA'].obsm['X_multimap']
    y_train = adata[adata.obs.batch=='RNA'].obs['celltype']
    X_test = adata[adata.obs.batch=='ATAC'].obsm['X_multimap']
    ...
    if save:
        ...
        # latent emb
        latent = pd.DataFrame(adata.obsm['X_multimap'], index=adata.obs_names)
