...
def main(): 
    ...
    multiatac.obs = pd.DataFrame({'celltype': 'unknown'}, index=atac_cell_names)
    multiatac.obs['dataset'] = 'Multiomics'
    del atac_counts, atac_gene_names, atac_cell_names

    # Concatenate RNA and ATAC in multiomics together
    atac = ad.concat([multiatac, atac])
    del multiatac
    ...
    n_multi = sum(atac.obs['dataset'] == 'Multiomics')
    priors[:n_multi, :n_multi] = np.eye(n_multi)
    ...
    # Get embeddings
    X_train = jm_data[0][n_multi:,:]
    X_test = jm_data[1][n_multi:,:]

    # KNN classifier
    y_train = rna.obs['celltype'][n_multi:]
    ...
    if save:
        # prob matrix
        prob = pd.DataFrame(knn.predict_proba(X_test), index=atac.obs_names[atac.obs['dataset'] != 'Multiomics'], columns=tmp)
        prob.to_csv(result_folder+'/prob.csv')
        # predicted label
        pred = prob.idxmax(axis=1)
        pred.to_csv(result_folder+'/pred.csv')
        # latent emb
        latent_rna = pd.DataFrame(X_train, index=rna.obs_names[rna.obs['dataset'] != 'Multiomics'])
        latent_atac = pd.DataFrame(X_test, index=atac.obs_names[atac.obs['dataset'] != 'Multiomics'])
