...
sys.path.append('/gpfs/gibbs/pi/zhao/yw599/Multiome/ATAC-annotation-benchmark/method_running/scJoint')
...
def run_scJoint(rna_path, atac_path, result_folder, subset_rna, subset_atac, rna_new_annot, stage1_lr, stage3_lr, nepoch, semi=True):
...
def main_scJoint(number_of_class, input_size, rna_paths, rna_labels, atac_paths, atac_labels, result_folder, stage1_lr, stage3_lr, nepoch, semi=True):
    ...
    for epoch in range(config.epochs_stage1):
        print('Epoch:', epoch)
        model_stage1.train(epoch, semi=semi)
    ...
    for epoch in range(config.epochs_stage3):
        print('Epoch:', epoch)
        model_stage3.train(epoch, semi=semi)
...
def main(): 
    ...
    run_scJoint(rna_path, atac_path, result_folder, subset_rna, subset_atac, rna_new_annot, stage1_lr, stage3_lr, nepoch, semi=False)
    # save results
    if result_folder[-1] != '/':
        result_folder += '/'
