...
def load_data(datadir,rgraph=True):
    ...
    data_train1, data_val1, label_train1, label_val1, lab_data2, lab_label2, types, data2_index = tuple(
        objects)
...
#     labels_test = np.array(label_test1).flatten()
    ...
    datas_tr = scipy.sparse.csr_matrix(datas_train.astype(float))
    datas_va = scipy.sparse.csr_matrix(datas_val.astype(float))
...
#     datas_te = scipy.sparse.csr_matrix(datas_test.astype('Float64'))
    ...
    features = sp.vstack((datas_tr, datas_va)).tolil()
...
#     labels_te = labels_test.flatten()
    ...
    labels = np.concatenate([labels_tr, labels_va])
...
#     labels_binary_test[test_mask, :] = new_label[test_mask, :]
    ...
    index1 = np.concatenate((data_train1.index, fake1, data_val1.index)).flatten()
    ...
    return adj, features, labels_binary_train, labels_binary_val, train_mask, pred_mask, val_mask, new_label, true_label, index_guide, rename, data2_index
