...
#             p_data[i], p_label[i], test_size=0.1, random_state=1)
        try:
            ...
            print('Cannot split train val! Setting val empty.')
            # import pdb; pdb.set_trace()
            temd_train, teml_train = p_data[i], p_label[i]
            temd_val = temd_train.loc[[], :]
            teml_val = teml_train.loc[[], :]
        print(temd_train.shape, teml_train.shape, temd_val.shape, teml_val.shape)
...
#     lab_train2 = pd.concat([label_train1, lab_label2])
    ...
    res = [
        data_train1, data_val1, label_train1,
        label_val1, lab_data2, lab_label2, types, data2_index
    ]
