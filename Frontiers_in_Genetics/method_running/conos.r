...
run.conos <- function(rna.path, activity.path, result.folder, subset.rna, subset.atac, annot.new.rna) {
    ...
    rna.data <- load.rna.folder(rna.path, subset.rna, annot.new.rna)
    rna.counts <- rna.data[['RNA']]@counts
    gene.activities.data <- load.atac.folder(activity.path, subset.atac)
    gene.activities <- gene.activities.data[['ATAC']]@counts
